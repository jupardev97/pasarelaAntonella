...
class modeloProveedor {
    ...
    public function selectAll() {
        $sql = 'SELECT * FROM pas_pro WHERE est_pro = "1" ';
        $this->db->query($sql);
...
//End findOne($data);
    ...
    public function create($data) {
        ...
        if ($this->db->execute()) {
            if ($this->db->rowCount() > 0) {
                $result['reg'] = $this->db->rowCount();
                $result['msg'] = 'Proveedor Agregado Correctamente';
                return $result;
            } else {
                $result['reg'] = 0;
