...
class modeloTipoInsumo {
    ...
    public function selectAll() {
        $sql = 'SELECT * FROM pas_tip_ins WHERE est_tip = 1';
        $this->db->query($sql);
    ...
    public function findOne($data) {
        $sql = 'SELECT * FROM pas_tip_ins WHERE ';
        $elements = 0;
...
//End findOne($data);
    ...
    public function create($data) {
        ...
        if ($this->db->execute()) {
            if ($this->db->rowCount() > 0) {
                $result['reg'] = $this->db->rowCount();
                $result['msg'] = 'Insumo Agregado Correctamente';
                return $result;
            } else {
                $result['reg'] = 0;
...
//End udate($data)
    public function delete( $data ){
        $sql = 'UPDATE pas_tip_ins SET est_tip = \'0\' WHERE ide_tip = :ide_tip';
        ...
        if ($this->db->execute()) {
            if ($this->db->rowCount() > 0) {
                $result['reg'] = $this->db->rowCount();
                $result['msg'] = 'Registro Eliminado Satisfactoriamente';
                return $result;
            } else {
                $result['reg'] = 0;
    ...
    public function drop($data) {
        $sql = 'DELETE FROM pas_tip_ins WHERE ';
